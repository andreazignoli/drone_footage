function [y1handle, y2handle, h] = fill_between(x, y1, y2, where, color, varargin);
...
for n = 1:max(cat)
    % ---------------------------------
    h(n) = fill_patch(x, y1, y2, cat == n, color);
    % ---------------------------------
end
...
hold on
% y1handle = plot(x,y1,'k-','linewidth', .1);
% y2handle = plot(x,y2,'k-','linewidth', .1);
...
function h = fill_patch(x, y1, y2, where, color);
